<template>
  <div class="m-dropdown">
    ...
    <div v-if="isShowDropdown && api" class="m-dropdown-data">
      <div
    ...
    <div v-if="isShowDropdown && dropdownItems" class="m-dropdown-data" :style="{ position: 'relative', bottom: '233px'}">
      <div v-for="item in dropdownItems" :key="item.value" class="m-dropdown-item" :class="checkSelected(value, item.name)" @click="selectDropdownOnClick(item.name,item.value)" :value="item.name">{{item.name}}</div>
    </div>
  </div>
</template>
...
export default {
  props: ["api", "dropdownData", "employee.DepartmentId","dropdownItems"],
  setup(props, { emit }) {
    ...
    if(props.dropdownItems){
      state.value = "40 bản ghi trên một trang"
      state.data = 40
    }
